...
{
    [RequireComponent(typeof(UICustomShopPack))]
    // quyết định xem pack này hiện mấy lần trong ngày và trong bao lâu
    public class PackSchedular : MonoBehaviour
    {
        [InfoBox("Lên kế hoạch xuất hiện pack trong ngày")]
        [SerializeField]
        ...
        [SerializeField] private bool resetExpireOnEnable = false;



        private readonly Service<TimeService> _timeService = new();
        private ShopItemKey _key;
        private UICustomShopPack _shopPack;
            ...
            else // Nếu chưa mua
            {
                ...
                if (CheckExpired() == false) // hết hạn
                {
                    ...
                    else
                    {
                        ...
                        var today = _timeService.Instance.GetCurrentTime().Date;
                        var endOfDay = today.AddDays(1).Subtract(DateTime.UnixEpoch).TotalSeconds;
                        _expiredTime.Value = (long)endOfDay;
                    }
                }
        ...
        {
            _shopPack = GetComponent<UICustomShopPack>();
            _key = _shopPack.ShopItemKey;
            ...
            if (_timeService.Instance.GetCurrentTime().Date != appearanceDate) // Nếu sang ngày mới thì reset data
            {
                ResetData();
                _appearanceDate.Value = _timeService.Instance.GetUnixTimeSeconds();
            }
        }
        ...
        public bool CheckExpired()
        {
            ...
            return _expiredTime.Value >= _timeService.Instance.GetUnixTimeSeconds();
        }
